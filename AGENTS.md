...
`yena-career-checkin` — Codex.ai 계정에 등록된 **진짜 서버 측 Routine**(trigger id
`trig_018URKHfpCgcEdGcMVgBtQKf`, cron `0 0 * * *` = 매일 09:00 KST), `growth_tree_counseling_protocol.md`를
그대로 실행. **2026-[REDACTED] 이전**에는 `23_app_Releaser/.Codex/schedule_registry.json`(로컬
PC 파일)에만 등록돼 있었는데, 이건 로컬 Codex가 켜져 있어야만 발화하는 방식이라
"로컬 PC 상태와 무관"하다는 이전 설명이 사실이 아니었다 — 사용자가 2026-[REDACTED]에 이 문제를
지적해서, `mcp__Claude_Code_Remote__create_trigger`로 Codex.ai 서버 Routine을 새로 만들고
로컬 registry 항목을 대체했다. 이제 로컬 PC/Codex Desktop이 꺼져 있어도 정시에 돈다.
**로컬 `23_app_Releaser/.Codex/schedule_registry.json`의 `yena-career-checkin` 항목은
...
- `ops-calendar/registry.json` — **돌기로 되어 있는 것**(기대). 캘린더
  ...
  `23_app_Releaser/.Codex/schedule_registry.json`, 이건 미러.
...
`ops-monitor-weekly` 루틴 등록. 이 저장소 세션에서는 Codex.ai 루틴 편집과 로컬 PC
작업에 접근할 수 없다.
...
- **멀티유저 로그인 + 각자 트리 구성 + 김근후 데이터 이전 + 히스토리 보존 + 사용자 자신의
  ...
  옵션을 먼저 확인할 것.
- **카드뉴스 콘텐츠를 이 사이트에 올리기**: 2026-08-20 19:11 사용자가 텔레그램으로 남긴
  메모("앞으로 카드뉴스는 그 ,,, 23 웹사이트에 올려야지") — 원문이 말줄임표로 끊겨 있어
  정확한 대상 트리(아마 `growth/geunhoo/` — 같은 날 세콤 채널에도 김근후 인플루언서 작업
  관련 메모가 왔음, `23_app_Releaser/files/Secom_Vision_Notion.md` §0 참고)나 형식(카드뉴스
  이미지 자산을 어디에 넣고 어떤 페이지에서 보여줄지)이 아직 불확실함 — 추측해서 만들지
  말고, 다음에 이 주제가 나오면 사용자에게 "어느 트리에, 어떤 형태로"부터 확인할 것.

...
| `action` | string | 구체적으로 뭘 하면 되는지. **추상적 조언 금지(사용자 지시, 2026-08-19)** — "꾸준히 올리세요", "진정성 있게 하세요" 같은 뜬구름 문장이 아니라, **이 사람이 그대로 따라 하면 실행이 되는 수준**으로 쓴다: 무엇을(구체적 소재/형식), 왜 그렇게 하는지(논리), 가능하면 참고할 실사례, 필요한 배경지식까지. `why`/`resources`/`personalIdeas` 필드를 적극 활용해서 이 구체성을 채운다 — `action` 한 줄로 안 되면 이 필드들로 나눠 담는다. |
